...
namespace AppWebInternetBanking.Views
{
    public partial class frmCredito : System.Web.UI.Page
    {
        ...
        private async void ObtenerDatosgrafico()
        {
            ...
            foreach (var c in credito.OrderByDescending(x => x.MontoCredito).Take(10))
            {
                string color = String.Format("#{0:X6}", random.Next(0x1000000));
                labels.Append(string.Format("'{0}',", c.Descripcion));
                data.Append(string.Format("'{0}',", c.MontoCredito));
